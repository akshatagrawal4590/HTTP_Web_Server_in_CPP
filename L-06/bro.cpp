...
class FileSystemUtility
{
    ...
    static bool fileExists(const char *path)
    {
        ...
        int x;
        x = stat(path, &s);
        if(x != 0) // To check if path exists
        {
            return false;
        }
        if(s.st_mode & S_IFDIR) // To check if the path is of file or folder
        {
            return false;
        }
        return true;
...
class Bro
{
    ...
    bool serveStaticResource(int clientSocketDescriptor, const char *requestURL)
    {
        // This means that if Bobby has to serve any static files then he has to put those files in a folder
        if(this -> staticResourcesFolder.length() == 0)
        {
            return false;
        }

        // If Bobby places the catch block elsewhere and he sends request to the server but the folder doesn't exist
        if(!FileSystemUtility::directoryExists(this -> staticResourcesFolder.c_str()))
        {
            return false;
        }
        string resourcePath = this -> staticResourcesFolder + string(requestURL);
        if(!FileSystemUtility::fileExists(resourcePath.c_str()))
        {
            return false;
        }
        FILE *file = fopen(resourcePath.c_str(), "rb");
        if(file == NULL) // The file exists but for some reason fopen fails to open file
        {
            return false;
        }
        long fileSize;
        ...
        rewind(file); // To move the the internal file pointer to the start of the file

        // Now if the filesize of the static file is not zero then we will serve chunks of 4096 bytes of the file
        char header[200];
        sprintf(header, "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: %d\r\nConnection: close\r\n\r\n", fileSize);
        send(clientSocketDescriptor, header, strlen(header), 0);
    ...
    void listen(int portNo, void (*callBack)(Error &))
    {
        ...
        while(1)
        {
            ...
            {
                if(!serveStaticResource(clientSocketDescriptor, requestURL))
                {
